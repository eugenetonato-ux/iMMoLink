...
@login_required
@require_POST
def annonce_retirer(request, pk):
    """Dépublie une annonce à la demande du propriétaire : elle repasse en
    brouillon et n'apparaît plus sur le site public."""
    if request.user.role != "proprietaire":
        return redirect("accounts:choix_role")
    if not profil_proprietaire_complet(request.user):
        return redirect("accounts:completer_profil_proprietaire")
    ...
    if annonce.statut == "publiee":
        services.retirer_annonce(annonce)
        messages.success(request, "Annonce retirée du marché. Tu peux la modifier et la renvoyer en validation.")
    else:
        messages.error(request, "Seule une annonce publiée peut être retirée.")
    return redirect("owner:mes_annonces")
...
def annonce_supprimer(request, pk):
    ...
    profil = request.user.ownerprofile
    annonce = get_object_or_404(Property, pk=pk, proprietaire=profil)
    if annonce.statut in ("brouillon", "refusee", "publiee"):
        annonce.delete()
        messages.success(request, "Annonce supprimée.")
    else:
        messages.error(request, "Impossible de supprimer une annonce en cours de validation.")
    return redirect("owner:mes_annonces")